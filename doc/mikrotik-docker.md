...
### 1. Menyipakan IP Docker `docker0`:
#### a. Installasi paket json
```bash
sudo apt update
sudo apt install -y jq
```

#### b. Check Version aplikasi
```bash
jq --version
```
...
#### b. Kemudian copy paste script dibawah ini
```bash
{
...
```
> Bila kosong normal, copy paste saja script diatas

### 3. Restart Docker Service Jalankan perintah berikut untuk menerapkan perubahan:
```bash
systemctl restart docker
```

### 4. Verifikasi Periksa apakah subnet baru telah diterapkan:
```bash
docker network inspect bridge
```

> Output akan menunjukkan subnet baru yang telah diterapkan, kurang lebih seperti dibawah ini.
```bash
[
    {
        "Name": "bridge",
...
# 3. Konfiurasi Jaringan Mikrotik Dan Docker
...
#### b. Simpan file hasil backup untuk referensi: File akan disimpan di direktori default MikroTik.
---
