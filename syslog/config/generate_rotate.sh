...
# File tempat mencatat proses generate (log "generate_rotate.sh")
GENERATE_LOG="/mnt/Data/Syslog/default/logrotate/generate.log"

# Pastikan owner dan group (jika mau di-set)
OWNER="abdullah"
GROUP="abdullah"

###############################################################################
# Fungsi log dengan info zona waktu
###############################################################################
log() {
  ...
  # Tulis ke layar dan juga ke file generate.log
...
###############################################################################
# Load pesan dari log_messages.json (jika ada)
###############################################################################
load_messages() {
...
if [[ ! -d "$LOG_ROOT" ]]; then
  log "$(get_message "generate.logroot_not_found")"
  exit 1
fi

if [[ -f "$CONFIG_FILE" ]]; then
  log "$(get_message "generate.config_exists")"
  log "Jika ingin membuat ulang, hapus atau backup terlebih dahulu file tersebut."
else
...
"$LOG_FILE" {
    ...
    postrotate
        # Graceful reload syslog-ng (sesuai container atau host)
